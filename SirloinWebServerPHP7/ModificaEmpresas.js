...
Ext.define('MyDesktop.ModificaEmpresas', {
	extend : 'Ext.ux.desktop.Module',
	requires : [ 'Ext.tab.Panel', 'Ext.chart.*', 'Ext.layout.container.Fit', 'Ext.fx.target.Sprite', 'Ext.selection.CheckboxModel' ],
	id : 'modificar-empresas',
	init : function() {
		this.launcher = {
			text : 'Modificar Empresas',
			id : 'launcher-modificar-empresas',
			iconCls : 'tabs'
		}
	},
			...
			items : [ {
				...
				items : [ {
					...
					items : [ {
						...
						tabIndex : 3,
						value:registro.data.nombre_empresa,
						allowBlank : false,
						name : 'nombre_empresa',
						anchor : '100%'
					}, {
...
//										panelEmpresas.items.get(0).items.get(0).items.get(3).setValue(storeColonias.getAt('0').get('id_colonia'));
		...
		if (!win) {
			win = desktop.createWindow({
				...
				onEsc : function() {
					var me = this;
					Ext.Msg.confirm('Confirmación de cerrado', '&iquest;Est&aacute;s seguro que deseas cerrar la ventana de Modificar Empresas?', function(btn) {
						if (btn == 'yes')
							me.destroy();
					});
				},
				...
				buttons : [ {
					...
					handler : function(btn) {
						...
						var datosFormulario = [ {
							...
							"id_empresa" : registro.internalId,
						...
						if (formulario.isValid()) {
							...
							formulario.submit({
								...
								url : 'php/saveFormModifyEmpr.php',
								params : {
