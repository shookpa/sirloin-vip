...
include_once("lib/conexionMysql.php");
...
setTitle ( "Reporte Excel con PHP y MySQL" )->setSubject ( "Reporte Excel con PHP y MySQL" )->setDescription ( $nombreArchivo )->setKeywords ( $nombreArchivo )->setCategory ( $nombreArchivo );
	...
	while ( ! $rsdDatos->EOF ) {
		$objPHPExcel->setActiveSheetIndex ( 0 )->setCellValue ( 'A' . $i, $rsdDatos->fields ["id"] )->setCellValue ( 'B' . $i, $rsdDatos->fields ["num_vip"] )->setCellValue ( 'C' . $i, utf8_encode ( $rsdDatos->fields ["nom_vip"] ) )->setCellValue ( 'D' . $i, ($rsdDatos->fields ["tel_vip"]) )->setCellValue ( 'E' . $i, ($rsdDatos->fields ["ema_vip"]) )->setCellValue ( 'F' . $i, ($rsdDatos->fields ["fec_vip"]) )->setCellValue ( 'G' . $i, ($rsdDatos->fields ["sal_vip"]) )->setCellValue ( 'H' . $i, ($rsdDatos->fields ["fuc_vip"]) )->setCellValue ( 'I' . $i, ($rsdDatos->fields ["muc_vip"]) )->setCellValue ( 'J' . $i, ($rsdDatos->fields ["fup_vip"]) )->setCellValue ( 'K' . $i, ($rsdDatos->fields ["mup_vip"]) )->setCellValue ( 'L' . $i, ($rsdDatos->fields ["tnr_vip"]) )->setCellValue ( 'M' . $i, ($rsdDatos->fields ["tmr_vip"]) )->setCellValue ( 'N' . $i, ($rsdDatos->fields ["tct_vip"]) )->setCellValue ( 'O' . $i, ($rsdDatos->fields ["pto_vip"]) )->setCellValue ( 'P' . $i, ($rsdDatos->fields ["pva_vip"]) )->setCellValue ( 'Q' . $i, ($rsdDatos->fields ["datetime_vip"]) );
		$i ++;
		$rsdDatos->MoveNext ();
	}
...
// 	$objWriter->save(dirname(__FILE__).'stats.xlsx');
	return 'http://sirloin.vip.soft-webcosmos.com.mx/resources/'.$nombreArchivo.".xlsx";
	;
// 	exit ();
}
?>
