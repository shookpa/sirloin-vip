 ...
 * FIN DE GRID
 */
Ext.define('MyDesktop.AdministracionVIP', {
	...
	init : function() {
		this.launcher = {
			...
			id : 'launcher-panel-administracion',
	...
	createWindow : function(aplicacion) {
		...
		var win = desktop.getWindow('panel-administracion');
...
//						allowBlank : false,
				...
				buttons : [ {
					...
					text : 'Exportar resultados de la búsqueda',
					tabIndex : 21,
					disabled: perm!=null && !perm.some(item => item.id_permiso == '3' && item.per_func4 == "1"),
					handler : function(btn) {
						var rest = this.up().up().down('#restaurante').value;
						var fec1 = this.up().up().down('#fecha1').getRawValue();
						var fec2 = this.up().up().down('#fecha2').getRawValue();
						...
						var tipo_registro = this.up().up().down('#tipo_registro_1').getValue();
//				
						// HACEMOS AJAX REQUEST POR AJAX: ESTA FUNCIONALIDAD ERA PARA CUANDO HUBIERA SELECCIONADOS:
...
//						});
//						jsonSelectos = JSON.stringify(selected);
//						console.debug(jsonSelectos);
//						Ext.Ajax.request({
//							url : 'php/lib/export_excel_mov_vip_todos.php',
//							method : 'POST',
//							params : {
//								"data" : jsonSelectos
//							},
...
//								Ext.MessageBox.show({
//									title : 'CREACION DE EXCEL EN DESARROLLO',
//									msg : "Exporta los " + selModel.getSelection().length + " registros seleccionados",
...
//							}
//						});
						//AHORA EXPORTAREMOS TODO LO QUE SE HAYA FILTRADO:
						window.open("php/lib/export_excel_mov_vip_todos.php?fec1=" + fec1 + '&fec2=' + fec2 + '&rest=' + rest + '&tipomov=' + tipo_mov + "&tarjeta=" + tarjeta + "&montoMax=" + montoMax + "&montoMin=" + montoMin + "&nombre=" + nombre + "&fechaNac=" + fechaNac+"&fechaNac2=" + fechaNac2+"&tipoReg=" + tipo_registro);
