...
Ext.define('MyDesktop.ModificaSucursales', {
	extend : 'Ext.ux.desktop.Module',
	requires : [ 'Ext.tab.Panel', 'Ext.chart.*', 'Ext.layout.container.Fit', 'Ext.fx.target.Sprite', 'Ext.selection.CheckboxModel' ],
	id : 'modificar-sucursales',
	init : function() {
		this.launcher = {
			text : 'Modificar Sucursales',
			id : 'launcher-modificar-sucursales',
			iconCls : 'tabs'
		}
	},
			...
			items : [ {
				...
				items : [ {
					...
					items : [ {
						...
						tabIndex : 3,
						allowBlank : false,
						value:registro.data.nombre_restaurante,
...
//								}//se comenta para que no espere el enter
					...
					// matchFieldWidth: false
						...
						xtype : 'combobox',
						anchor : '100%',
						value:registro.data.idmunicipio,
						allowBlank : false,
						matchFieldWidth : false,
						emptyText : 'Selecciona...',
						queryMode : 'local',
						...
						allowBlank : false,
						xtype : 'textfield',
						value:registro.data.nombreCalle,
						name : 'calleCot',
						// id: 'calleCot',
						anchor : '100%'
					}, {
		...
		if (!win) {
			win = desktop.createWindow({
				...
				onEsc : function() {
					var me = this;
					Ext.Msg.confirm('Confirmación de cerrado', '&iquest;Est&aacute;s seguro que deseas cerrar la ventana de Modificar Sucursales?', function(btn) {
						if (btn == 'yes')
							me.destroy();
					});
				},
				...
				buttons : [ {
					...
					handler : function(btn) {
						...
						var datosFormulario = [ {
							...
							"id_restaurante" : registro.internalId
						} ];
			...
			//QUE CARGUE LAS DIRECCIONES QUE VIENE POR AJAX:
			storeEstados.load({
				params : {
					tabla : 'estados',
					filtros : 'INNER JOIN cp ON cp.id_estado = estados.id_estado WHERE cp.cp =' + registro.data.cp
				},
				callback : function(records, operation, success) {
					// DESPUES DE QUE ACABE DE
					// PINTAR EL STORE DEL ESTADO
			...
			storeMunicipios.load({
				params : {
					tabla : 'municipios',
					filtros : 'INNER JOIN cp ON cp.id_municipio = municipios.id_municipio WHERE cp.cp =' + registro.data.cp
				},
				callback : function(records, operation, success) {
					// DESPUES DE QUE ACABE DE
					// PINTAR EL STORE DEL MUNICIPIO
