...
//         }
    ...
    public function isHTML($isHtml = true)
    {
        if ($isHtml) {
            ...
            $this->ContentType = CONTENT_TYPE_PLAINTEXT;
        }
    }
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if ($this->alternativeExists()) {
                $this->ContentType = CONTENT_TYPE_MULTIPART_ALTERNATIVE;
            }
...
//             }
            ...
            if (!empty($this->DKIM_domain)
                ...
                $this->normalizeBreaks($header_dkim) . $LE;
            }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (!preg_match(
                ...
                edebug($this->lang('connect_host') . ' ' . $hostentry);
...
//             $sslext = true;
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        $PHPMAILER_LANG = array(
            ...
            'data_not_accepted' => 'SMTP Error: data not accepted.',
        		'empty_message' => 'Message body empty:'.$this->Body."En serio?",
    ...
    public function addrAppend($type, $addr)
    {
        ...
        return $type . ': ' . implode(', ', $addresses) . $LE;
    }
    ...
    public function createHeader()
    {
        ...
        $result .= $this->addrAppend('From', array(trim($this->From), $this->FromName));//2 NIVELES DE ARRAY, SOLO DEJE UNO JCENTENO
    ...
    public function getMailMIME()
    {
        ...
        switch ($this->message_type) {
            case 'inline':
                $result .= $this->headerLine('Content-Type', CONTENT_TYPE_MULTIPART_RELATED . ';');
            ...
            case 'alt_attach':
            case 'alt_inline_attach':
                $result .= $this->headerLine('Content-Type', CONTENT_TYPE_MULTIPART_MIXED . ';');
                $result .= $this->textLine("\tboundary=\"" . $this->boundary[1] . '"');
                break;
            case 'alt':
            case 'alt_inline':
                $result .= $this->headerLine('Content-Type', CONTENT_TYPE_MULTIPART_ALTERNATIVE . ';');
                $result .= $this->textLine("\tboundary=\"" . $this->boundary[1] . '"');
                break;
            default:
        ...
        if (ENCODING_7BIT != $this->Encoding) {
            // RFC 2045 section 6.4 says multipart MIME parts may only use 7bit, 8bit or binary CTE
            if ($ismultipart) {
                if (ENCODING_8BIT == $this->Encoding) {
                    $result .= $this->headerLine('Content-Transfer-Encoding', ENCODING_8BIT);
                }
    ...
    public function createBody()
    {
        ...
        if (ENCODING_8BIT == $bodyEncoding and !$this->has8bitChars($this->Body)) {
            $bodyEncoding = ENCODING_7BIT;
            //All ISO 8859, Windows codepage and UTF-8 charsets are ascii compatible up to 7-bit
            $bodyCharSet = 'us-ascii';
        }
        //If lines are too long, and we're not already using an encoding that will shorten them,
        //change to quoted-printable transfer encoding for the body part only
        if (ENCODING_BASE64 != $this->Encoding and $this->hasLineLongerThanMax($this->Body)) {
            $bodyEncoding = ENCODING_QUOTED_PRINTABLE;
        }

        $altBodyEncoding = $this->Encoding;
        $altBodyCharSet = $this->CharSet;
        //Can we do a 7-bit downgrade?
        if (ENCODING_8BIT == $altBodyEncoding and !$this->has8bitChars($this->AltBody)) {
            $altBodyEncoding = ENCODING_7BIT;
            //All ISO 8859, Windows codepage and UTF-8 charsets are ascii compatible up to 7-bit
            $altBodyCharSet = 'us-ascii';
        }
        //If lines are too long, and we're not already using an encoding that will shorten them,
        //change to quoted-printable transfer encoding for the alt body part only
        if (ENCODING_BASE64 != $altBodyEncoding and $this->hasLineLongerThanMax($this->AltBody)) {
            $altBodyEncoding = ENCODING_QUOTED_PRINTABLE;
        }
        //Use this as a preamble in all multipart message types
        $mimepre = 'This is a multi-part message in MIME format.' . $LE;
        switch ($this->message_type) {
            case 'inline':
                $body .= $mimepre;
            ...
            case 'inline_attach':
                ...
                $body .= $this->headerLine('Content-Type', CONTENT_TYPE_MULTIPART_RELATED . ';');
            ...
            case 'alt':
                $body .= $mimepre;
                $body .= $this->getBoundary($this->boundary[1], $altBodyCharSet, CONTENT_TYPE_PLAINTEXT, $altBodyEncoding);
                $body .= $this->encodeString($this->AltBody, $altBodyEncoding);
                $body .= $LE;
                $body .= $this->getBoundary($this->boundary[1], $bodyCharSet, CONTENT_TYPE_TEXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
                $body .= $LE;
                if (!empty($this->Ical)) {
                    $body .= $this->getBoundary($this->boundary[1], '', CONTENT_TYPE_TEXT_CALENDAR . '; method=REQUEST', '');
            ...
            case 'alt_inline':
                ...
                $body .= $this->textLine("\tboundary=\"" . $this->boundary[2] . '"');
                $body .= $LE;
                $body .= $this->getBoundary($this->boundary[2], $bodyCharSet, CONTENT_TYPE_TEXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
                $body .= $LE;
                $body .= $this->attachAll('inline', $this->boundary[2]);
                $body .= $LE;
            ...
            case 'alt_attach':
                $body .= $mimepre;
                $body .= $this->textLine('--' . $this->boundary[1]);
                $body .= $this->headerLine('Content-Type', CONTENT_TYPE_MULTIPART_ALTERNATIVE . ';');
                $body .= $this->textLine("\tboundary=\"" . $this->boundary[2] . '"');
                $body .= $LE;
                $body .= $this->getBoundary($this->boundary[2], $altBodyCharSet, CONTENT_TYPE_PLAINTEXT, $altBodyEncoding);
                $body .= $this->encodeString($this->AltBody, $altBodyEncoding);
                $body .= $LE;
                $body .= $this->getBoundary($this->boundary[2], $bodyCharSet, CONTENT_TYPE_TEXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
                $body .= $LE;
                if (!empty($this->Ical)) {
                    $body .= $this->getBoundary($this->boundary[2], '', CONTENT_TYPE_TEXT_CALENDAR . '; method=REQUEST', '');
                    $body .= $this->encodeString($this->Ical, $this->Encoding);
                }
            ...
            case 'alt_inline_attach':
                ...
                $body .= $LE;
                $body .= $this->getBoundary($this->boundary[3], $bodyCharSet, CONTENT_TYPE_TEXT_HTML, $bodyEncoding);
                $body .= $this->encodeString($this->Body, $bodyEncoding);
                $body .= $LE;
...
//             }
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] == $disposition_type) {
                ...
                // RFC1341 part 5 says 7bit is assumed if not specified
                if (ENCODING_7BIT != $encoding) {
                    $mime[] = sprintf('Content-Transfer-Encoding: %s%s', $encoding, $LE);
                }

                if (!empty($cid)) {
                    $mime[] = sprintf('Content-ID: <%s>%s', $cid, $LE);
                }
    ...
    public function encodeHeader($str, $position = 'text')
    {
        ...
        $maxlen = STD_LINE_LENGTH - $lengthsub;
        ...
        if ($matchcount > strlen($str) / 3) {
            ...
            // - charset name length
            $maxlen = STD_LINE_LENGTH - $lengthsub - 8 - strlen($this->CharSet);
            ...
            if ($str == $encoded) {
                ...
                $encoded = trim(chunk_split($str, STD_LINE_LENGTH, $LE));
            }
            $encoded = str_replace($LE, "\n", trim($encoded));
...
//             }
//         );
        //ESTA LOGICA LA CAMBIAMOS Y SE LA DEJAMOS A MIR PROGRAMAS JCENTENO
        $this->RecipientsQueue =$this->RecipientsQueue;
    }
        ...
        if (array_key_exists(2, $images)) {
            ...
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (preg_match('#^data:(image/(?:jpe?g|gif|png));?(base64)?,(.+)#', $url, $match)) {
                    if (count($match) == 4 and ENCODING_BASE64 == $match[2]) {
                    ...
                    if (!$this->cidExists($cid)) {
                        $this->addStringEmbeddedImage($data, $cid, 'embed' . $imgindex, ENCODING_BASE64, $match[1]);
                    }
                    $message = str_replace(
                        $images[0][$imgindex],
                ...
                if (// Only process relative URLs if a basedir is provided (i.e. no absolute local paths)
                    ...
                    if ($this->addEmbeddedImage(
                        ...
                        $filename,
                        ENCODING_BASE64,
                        _mime_types((string) mb_pathinfo($filename, PATHINFO_EXTENSION))
                    )
                    ) {
    ...
    public static function filenameToType($filename)
    {
        ...
        $ext = mb_pathinfo($filename, PATHINFO_EXTENSION);
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
        ...
        $headers = explode($LE, $headers_line);
        $from_header = '';
        $to_header = '';
        $date_header = '';
        $current = '';
        ...
        return $this->normalizeBreaks($dkimhdrs . $signed) . $LE;
    }
    ...
    {
        return (bool) preg_match('/^(.{' . (self::MAX_LINE_LENGTH + strlen($LE)) . ',})/m', $str);
    }
