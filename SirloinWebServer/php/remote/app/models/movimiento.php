...
class Movimiento extends Model {
	...
	static function find($id) {
		global $dbh;
		$found = null;
		foreach ( $dbh->rs ( "clientes_vip", "id" ) as $rec ) {
			if ($rec ['id'] == $id) {
				$found = new self ( $rec );
				break;
			}
	...
	static function update($id, $params) {
		...
		foreach ( $rs as $idx => $row ) {
			if ($row ['id'] == $id) {
				...
				$dbh->update ( "clientes_vip", $rec->attributes, "id" );
				break;
			}
		}
		return $rec;
	...
	static function destroy($id) {
		...
		foreach ( $rs as $idx => $row ) {
			if ($row ['id'] == $id) {
				$rec = new self ( $dbh->destroy ( "clientes_vip", "id", $id ) );
				break;
			}
		}
		return $rec;
	...
	static function searchMovimientos() {
		...
		if (isset ( $_REQUEST ["mesNac"] )) {
			$mesNac = $_REQUEST ["mesNac"];
		...
		if ($muestraMovs)
			return $dbh->rs ( "mov_vip
				...
				LEFT JOIN usuarios ON usuarios.num_vip = clientes_vip.num_vip
				WHERE 1
					$condFec $condRest  $condTarj $condNombre $condSaldo  $condFecApertura $condTel $condEmail $condCP $condFecNac $condMesNac $condTipoMov $condFecMov 
	...
	static function searchMovimientosPaginados() {
		...
		//mov_vip.num_vip, mov_vip.det_vip, mov_vip.fec_vip
		if ($muestraMovs)
			return $dbh->rs ( "mov_vip
														INNER JOIN clientes_vip ON clientes_vip.num_vip=mov_vip.num_vip
														LEFT JOIN cat_restaurantes ON mov_vip.id_restaurante = cat_restaurantes.id_restaurante
	...
	public function save() {
		global $dbh;
		$dbh->insert ( $this->attributes, "clientes_vip" );
	}
